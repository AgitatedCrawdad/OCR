...
def write(image,index,type):
    ...
    file = open(path+r'\text_' + str(image.string_count) + '.txt', 'w')
...
def main():
    ...
    images_b, images_s = throw_away.main('throw_away_dir')

    for image in images_s:
        write_text(image.text)
        text = read_text()
        if text:
            count = string_count(text)
...
if __name__ == '__main__':
    ...
    images_b, images_s = main()
    
    test_s = sorted(range(len(images_s)), key=lambda i: images_s[i].string_count)[-20:]
    test_b = sorted(range(len(images_b)), key=lambda i: images_b[i].string_count)[-20:]

    for index in test_s:
        write(images_s[index],index,'scanned')

    for index in test_b:
        write(images_b[index],index,'binarized')
